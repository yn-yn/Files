...
人工神经网络是一个智能系统，它由许多简单且高度相互连接的处理元素组成。这些元素通过其对外部输入的动态状态反应能力来传递信息。人工神经网络基本上由相互连接的节点的良好组织层显示。节点包括一个激活函数。NN机制的输入层可以识别图12中的模式。这些模式然后通过一个加权连接系统传达给隐藏层进行实际处理。隐蔽层与输出层连接，给出所需答案。NN的特点，如泛化能力、大规模并行性、分布式表示、学习能力和容错性，使其在移动机器人导航领域非常有用。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTMwMzYxMzQ2M119
-->